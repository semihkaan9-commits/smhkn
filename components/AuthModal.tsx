...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onLogin, onRegister }) => {
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    if (!name.trim() || !surname.trim()) {
      alert("Giriş yapmak için İsim ve Soyisim alanlarını doldurmalısınız.");
      return;
    }

    // Determine Role based on Radio Selection (Defaults to Villager in UI)
    const isVillagerLogin = document.getElementById('login-type-villager') as HTMLInputElement;
    const role = isVillagerLogin?.checked ? UserRole.VILLAGER : UserRole.GUEST;
  ...
  const handleRegisterSubmit = (e: React.FormEvent) => {
    ...
    if (!name.trim() || !surname.trim() || !email.trim()) {
        alert("Lütfen isim, soyisim ve e-posta alanlarını doldurunuz.");
        return;
    }
    ...
    if (registerType === UserRole.VILLAGER) {
        if (!profession.trim() || !contact.trim() || !address.trim()) {
            alert("Köy sakini kaydı için Meslek, İletişim ve Adres alanları zorunludur.");
            return;
        }
    }
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-4">
               <div className="space-y-4">
                 <div>
                   ...
                   <div className="flex gap-4 mt-1">
                     <label className="flex items-center gap-2 cursor-pointer">
                       <input type="radio" name="loginType" id="login-type-villager" className="text-green-600 focus:ring-green-500" defaultChecked />
                       <span>Köylü</span>
                     </label>
                     <label className="flex items-center gap-2 cursor-pointer">
                       <input type="radio" name="loginType" value="guest" className="text-green-600 focus:ring-green-500" />
                       <span>Misafir</span>
                     </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                       <input type="radio" name="loginType" value="admin" className="text-green-600 focus:ring-green-500" onChange={(e) => {
                         if(e.target.checked) {
                           setUsername('admin'); 
                         }
                       }} />
                 ...
                 <div className="grid grid-cols-2 gap-4">
                   <div>
                     <label className="block text-sm font-medium text-gray-700">İsim / K.Adı</label>
                     <input 
                        type="text" 
                        value={username || name} 
                   ...
                   <div>
                     <label className="block text-sm font-medium text-gray-700">Soyisim / Şifre</label>
                     <input 
