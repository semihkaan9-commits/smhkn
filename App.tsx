...
const App: React.FC = () => {
  ...
  const refreshData = async () => {
    try {
      ...
      if (villagersData && villagersData.length > 0) {
      ...
      if (guestsData) {
        const mappedGuests: AnyUser[] = guestsData.map((g: any) => ({
          ...
          role: UserRole.GUEST,
          email: undefined // or g.email if available and needed
        }));
      ...
      if (eventsData && eventsData.length > 0) {
        const mappedEvents = eventsData.map((item: any) => ({
          ...item,
          imageUrl: item.image_url || item.imageUrl,
          startDate: item.start_date || item.startDate, // Handle snake_case if DB uses it
